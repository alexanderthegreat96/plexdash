...
//				chmod(PLPP_CONFIGURATION_PATH.$key.'.json', 0666);
	...
	if (empty($plppConfiguration['general']['script_guid'])) {
		...
		if ($success) {
			$plppNotifications[count($plppNotifications)] = 'Configuration file "'.PLPP_CONFIGURATION_PATH.'general.json" successfully saved!';
		}
		else {
			$plppErrors[count($plppErrors)] = 'Unable to save configuration file "'.PLPP_CONFIGURATION_PATH.'general.json"!';
		}
	}
	...
	if (empty($plppConfiguration['usersettings']['admin_password'])) {
		...
		$plppOutput['Content'] .= '		<div class="col-md-4">'.PHP_EOL;
		$plppOutput['Content'] .= '			<input id="password" name="settings_password" type="settings_password" placeholder="" value="" class="form-control input-md" required="">'.PHP_EOL;
